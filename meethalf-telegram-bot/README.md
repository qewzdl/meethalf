...
(`/shadow_ban <user_id|@username>` and `/shadow_unban <user_id|@username>`), plus `Hide profile` / `Show profile` / `Delete profile`
(`/hide_profile <user_id|@username>`, `/show_profile <user_id|@username>`, and `/delete_profile <user_id|@username>`). It also supports `Grant moderator` / `Revoke moderator` actions
...
`Ban a user` / `Lift ban`, `Shadow ban` / `Lift shadow ban`, `Hide profile` / `Show profile` / `Delete profile` (regular users only), and `Clear reports`.
