...
menu that opens admin management. The admin dashboard supports `All users`, `Banned list`, `Moderator list`, and `Reports`
...
`/ban <user_id|@username>` and `/unban <user_id|@username>`). It also supports `Grant moderator` / `Revoke moderator` actions
...
`Moderator dashboard` button, but only with `All users`, `Banned list`, and `Reports` lists plus `Ban a user` / `Lift ban`
