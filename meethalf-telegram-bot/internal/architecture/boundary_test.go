...
const (
	modulePath      = "meethalf-telegram-bot"
	otherModulePath = "meethalf-api"
)
...
func TestInternalBoundaries(t *testing.T) {
	...
	walkErr := filepath.WalkDir(internalRoot, func(path string, entry fs.DirEntry, err error) error {
		...
		layerName, fileSubLayer := layerFromRel(relPath)
		...
		for _, importPath := range imports {
			...
			importLayer, importSubLayer, ok := layerForImport(importPath)
			if !ok {
				continue
			}
			...
			if fileLayer == layerTransport && importLayer == layerTransport && transportSubLayerMismatch(fileSubLayer, importSubLayer) {
				violations = append(violations, fmt.Sprintf("%s: transport/%s imports transport/%s (%s)", relPath, fileSubLayer, importSubLayer, importPath))
			}
		}
...
func transportSubLayerMismatch(fileSubLayer, importSubLayer string) bool {
	if !isTransportSubLayer(fileSubLayer) || !isTransportSubLayer(importSubLayer) {
		return false
	}
	return fileSubLayer != importSubLayer
}

func isTransportSubLayer(name string) bool {
	return name == "api" || name == "telegram"
}
